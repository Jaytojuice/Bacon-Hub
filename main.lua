
if game.PlaceId == 286090429 then -- Arsenal
     loadstring(game:HttpGet(('https://raw.githubusercontent.com/Jaytojuice/Bacon-Hub/main/Games/Arsenal'),true))()   
end
if game.PlaceId == 4972091010 then -- Zombie Uprising
      loadstring(game:HttpGet(('https://raw.githubusercontent.com/Jaytojuice/Bacon-Hub/main/Games/Zombie_Uprising'),true))() 
end
if game.PlaceId == 142823291 then -- MM2
      loadstring(game:HttpGet(('https://raw.githubusercontent.com/Jaytojuice/Bacon-Hub/main/Games/MM2'),true))() 
      end
